class Discussion < ActiveRecord::Base
  ...
  def self.create_xml(user, discussion, user_filters, users_assigned, answers)
    if user.participant 
      ...
      xml_data = []
      xml_data << {:user_name => user.name, :user_id => user.id, :admin => user_criteria(user), :image_path => discussion.media.url, :discussion_id => discussion.id, :discussion_users => users_assigned.size, :answers => answers.size, :w => "440", :h => "310"} 
