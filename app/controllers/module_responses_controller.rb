class ModuleResponsesController < ApplicationController
  ...
  def get_module
    ...
    respond_to do |format|
      ...
      isadmin=self.current_user.admin || self.current_user.client ?   true : false
      flex_module= {:module_id => module_info.id, :module_type => module_info.module_type.name, :is_admin => isadmin  , :is_moderator =>  self.current_user.moderator, :discussion_id => module_info.discussion.id}
